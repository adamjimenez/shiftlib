...
jQuery.ajax(url,{dataType:'json',type:'post',data:jQuery(this).serializeAll()+'&validate=1&nospam=1',success:jQuery.proxy(function(returned){var errorMethod='inline';if(jQuery(this).attr('data-errorMethod')){errorMethod=jQuery(this).attr('data-errorMethod');}else if(jQuery(this).attr('errorMethod')){errorMethod=jQuery(this).attr('errorMethod');}
debug(returned);showProgress(false);if(parseInt(returned,10)!==returned-0){if(returned.length>0){var errors='';for(i=0;i<returned.length;i++){var pos=returned[i].indexOf(' ');if(pos===-1){var field=returned[i];var error='Required';}else{var field=returned[i].substring(0,pos);var error=returned[i].substring(pos+1);}
var parent='';if(this[field]){if(this[field].style){if(!firstError){var firstError=field;}
parent=this[field].parentNode;}else if(this[field][0]){parent=this[field][0].parentNode.parentNode;}
errors+=field+'\n';}else if(this[field+'[]']){if(this[field+'[]'].style){if(!firstError){var firstError=field;}
parent=this[field+'[]'].parentNode;}else if(this[field+'[]'][0]){parent=this[field+'[]'][0].parentNode.parentNode;}
errors+=field+'\n';}else{errors+=error+'\n';}
